...
const AboutSection = () => {
  const stats = [
    { icon: Heart, value: '500+', label: 'Clients satisfaits', color: 'text-pink-500', bg: 'bg-pink-100', suffix: '+' },
    { icon: Award, value: '10+', label: "Années d'expérience", color: 'text-yellow-500', bg: 'bg-yellow-100', suffix: '+' },
    { icon: Users, value: '50+', label: 'Artisans partenaires', color: 'text-blue-500', bg: 'bg-blue-100', suffix: '+' },
    { icon: TrendingUp, value: '98', label: 'Taux de satisfaction', color: 'text-green-500', bg: 'bg-green-100', suffix: '%' },
  ];

  const values = [
    { icon: Leaf, title: 'Authenticité', description: 'Des produits 100% artisanaux faits main', color: 'bg-green-100', iconColor: 'text-green-600' },
    { icon: Globe, title: 'Made in Africa', description: 'Valorisation du savoir-faire local', color: 'bg-yellow-100', iconColor: 'text-yellow-600' },
    { icon: Heart, title: 'Passion', description: 'Des artisans passionnés par leur métier', color: 'bg-red-100', iconColor: 'text-red-600' },
    { icon: Target, title: 'Qualité', description: 'Sélection rigoureuse des matériaux', color: 'bg-green-100', iconColor: 'text-green-600' },
  ];
    ...
    <section className="py-20 bg-white">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          {/* Partie gauche - Image avec badges */}
          <ScrollReveal direction="left" delay={200}>
            <div className="relative group">
              ...
              <div className="absolute -bottom-6 -left-6 bg-white rounded-2xl p-4 shadow-xl flex items-center gap-3 animate-bounce-slow">
                ...
                <div>
                  <p className="font-bold text-gray-800">Notée 4.9/5</p>
              ...
              <div className="absolute -top-6 -right-6 bg-white rounded-2xl p-4 shadow-xl flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-gray-800">Livraison offerte</p>
                  <p className="text-xs text-gray-500">dès 50 000 FCFA</p>
                </div>
              </div>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal direction="right" delay={300}>
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {stats.map((stat, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:shadow-md transition group hover:-translate-y-1 duration-300">
                    <div className={`${stat.bg} p-2 rounded-full group-hover:scale-110 transition`}>
                      <stat.icon className={`w-5 h-5 ${stat.color}`} />
                    </div>
                    <div>
                      <p className="font-bold text-gray-800 text-lg">{stat.value}{stat.suffix}</p>
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {values.map((value, idx) => (
                    <div key={idx} className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 transition hover:-translate-y-1 duration-300">
                      <div className={`${value.color} p-1.5 rounded-full`}>
                        <value.icon className={`w-3 h-3 ${value.iconColor}`} />
                      </div>
                      <div>
                        <p className="font-semibold text-gray-800 text-sm">{value.title}</p>
              ...
              <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-green-50 to-yellow-50 rounded-xl border border-green-200 hover:shadow-md transition duration-300">
                <Verified className="w-8 h-8 text-green-600" />
                <div>
                  <p className="font-semibold text-gray-800">100% produits artisanaux authentiques</p>
                  <p className="text-gray-500 text-sm">Chaque pièce est unique et faite avec passion</p>
                </div>
              </div>
            </div>
          </ScrollReveal>
        ...
        <ScrollReveal direction="up" delay={400}>
          <div className="mt-20">
            <div className="text-center mb-10">
              <h3 className="text-2xl font-bold text-gray-800">Notre parcours</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {milestones.map((milestone, idx) => (
                <div key={idx} className="text-center group hover:-translate-y-2 transition duration-300">
                  <div className="w-16 h-16 bg-gradient-to-br from-green-100 to-yellow-100 rounded-2xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition duration-300 shadow-md">
                    <milestone.icon className="w-8 h-8 text-green-600" />
                  </div>
                  <div className="text-green-600 font-bold text-lg">{milestone.year}</div>
                  <h4 className="font-semibold text-gray-800 text-sm mt-1">{milestone.title}</h4>
