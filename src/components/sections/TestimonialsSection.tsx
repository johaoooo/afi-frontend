        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="w-11 h-11 rounded-full border-2 border-gray-200 hover:border-[#1a6b3c] hover:text-[#1a6b3c] flex items-center justify-center transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1a6b3c]"
            >
            ...
            <button
              ...
              className="w-11 h-11 rounded-full bg-[#1a6b3c] hover:bg-[#14532d] text-white flex items-center justify-center transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1a6b3c] focus-visible:ring-offset-2"
            >
          ...
          >
            <div className="flex gap-1 mb-8">
              {Array.from({ length: t.rating }).map((_, i) => (
                <FiStar key={i} className="w-5 h-5 text-[#1a6b3c] fill-current" aria-hidden="true" />
              ))}
            </div>

            <p className="text-2xl md:text-3xl font-medium text-gray-700 leading-snug mb-10">
              « {t.text} »
            </p>

            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-[#1a6b3c]/10 overflow-hidden shrink-0">
                <img
                  src={t.image}
                  alt={t.name}
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(t.name)}&background=1a6b3c&color=fff`;
                  }}
                />
              </div>
              <div>
