...
const reasons = [
  { key: 'authentic', icon: FiHeart },
  { key: 'diversity', icon: FiGlobe },
  { key: 'quality', icon: FiStar },
  { key: 'durability', icon: FiClock },
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white tracking-tight leading-[1.05] mt-3">
            ...
            <br />
            <span className="text-green-200">{t('why_choose.highlight')}</span>
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {reasons.map((item, i) => (
            ...
            >
              ...
              <div>
                <h3 className="font-black text-white text-lg leading-tight">{t(`why_choose.${item.key}`)}</h3>
                <p className="text-green-100/80 text-sm mt-1.5 leading-relaxed">{t(`why_choose.${item.key}_desc`)}</p>
              </div>
            </motion.div>
          ))}
        </div>
